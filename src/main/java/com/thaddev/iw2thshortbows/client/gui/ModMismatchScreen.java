...
public class ModMismatchScreen extends Screen {
    ...
    public ModMismatchScreen(String clientModVersion, String serverModVersion, String clientModLoader, String serverModLoader) {
        super(new TranslatableComponent(IWant2TryHardsShortbows.SCREEN_VERSION_MISMATCH));
    ...
    protected void init() {
        ...
        this.addRenderableWidget(drawCenteredButton((this.width / 2) - 160, pos + 25, 150, 20, true, new TranslatableComponent(OPEN_CURSEFORGE),
                ...
                (pButton, pPoseStack, pMouseX, pMouseY) ->
                    renderTooltip(pPoseStack, new TranslatableComponent(OPEN_CURSEFORGE), pMouseX, pMouseY)
            )
        );
        this.addRenderableWidget(drawCenteredButton((this.width / 2), pos + 25, 150, 20, true, new TranslatableComponent(OPEN_MODRINTH),
                ...
                (pButton, pPoseStack, pMouseX, pMouseY) ->
                    renderTooltip(pPoseStack, new TranslatableComponent(OPEN_MODRINTH), pMouseX, pMouseY)
            )
        );
        this.addRenderableWidget(drawCenteredButton((this.width / 2) + 160, pos + 25, 150, 20, true, new TranslatableComponent(OPEN_GITHUB),
                (pButton) ->
                    this.minecraft.setScreen(new ConfirmLinkScreen((p_169337_) -> {
                        if (p_169337_) {
                ...
                (pButton, pPoseStack, pMouseX, pMouseY) ->
                    renderTooltip(pPoseStack, new TranslatableComponent(OPEN_GITHUB), pMouseX, pMouseY)
            )
        );
        this.addRenderableWidget(drawCenteredButton((this.width / 2) - 105, pos + 55, 200, 20, true, new TranslatableComponent(DISCONNECT),
                (pButton) -> {
        ...
        this.addRenderableWidget(drawCenteredButton((this.width / 2) + 105, pos + 55, 200, 20, true, new TranslatableComponent(IGNORE),
                (pButton) -> {
                    if (pButton.getMessage().getString().equals(new TranslatableComponent(IGNORE).getString())) {
                        pButton.setMessage(new TranslatableComponent(IGNORE_CONFIRM).withStyle(ChatFormatting.RED));
                    }else{
                        this.onClose();
                    }
                },
                (pButton, pPoseStack, pMouseX, pMouseY) ->
                    renderTooltip(pPoseStack, new TranslatableComponent(IGNORE_CONFIRM), pMouseX, pMouseY)
            )
        );
    }
    ...
    public void render(@NotNull PoseStack matrixStack, int p_96563_, int p_96564_, float p_96565_) {
        ...
        drawCenteredString(matrixStack, this.font, new TranslatableComponent(MESSAGE_1), this.width / 2, this.height / 2 - 45, 16777215);
        drawCenteredString(matrixStack, this.font, new TranslatableComponent(MESSAGE_2), this.width / 2, this.height / 2 - 25, 16777215);
        drawCenteredString(matrixStack, this.font, new TranslatableComponent(MESSAGE_DISCREPANCY_LIST), this.width / 2, this.height / 2 - 5, 16777215);
        int pos = this.height / 2 + 15;
        if (!serverModLoader.equals(clientModLoader)) {
            MutableComponent toShow = new TranslatableComponent(MESSAGE_MODLOADER)
                .append("  ")
                .append(
                    new TranslatableComponent(CLIENT)
                        .append(" (")
                ...
                .append(" -> ")
                .append(
                    new TranslatableComponent(SERVER)
                        .append(" (")
        ...
        if (!serverModVersion.equals(clientModVersion)) {
            MutableComponent toShow = new TranslatableComponent(MESSAGE_MODVERSION)
                .append("  ")
                .append(
                    new TranslatableComponent(CLIENT)
                        .append(" (")
